...
&usage() unless (scalar(@ARGV) == 4 and $ARGV[3] =~ /cds|pep/i);

my $taxoncode = $ARGV[0];
my $inputfile = $ARGV[1];
my $idField = $ARGV[2];
my $seqtype = lc($ARGV[3]);
open(IN, $inputfile) || die "Can't open input file '$inputfile'\n";
open(OUT, ">$taxoncode.$seqtype.fasta") || die "Can't open output file '$taxoncode.fasta'\n";
...
EXAMPLE: orthomclSoftware/bin/orthomclAdjustFasta hsa Homo_sapiens.NCBI36.53.pep.all.fa 1 pep
